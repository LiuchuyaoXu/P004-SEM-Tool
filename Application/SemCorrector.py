...
class SemCorrector:
    ...
    def iterate(self):
        thread = threading.Thread(target=self.iterateThread)
        thread.start()

    def iterateThread(self):
        print("--------------------")
        ...
        image = self.sem.grabImage(x=self.rasterX, y=self.rasterY, width=self.rasterWidth, height=self.rasterHeight)
